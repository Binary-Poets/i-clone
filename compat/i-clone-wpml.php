<?php
add_action( 'admin_init', 'i_clone_wpml_init' );

function i_clone_wpml_init() {
	if ( defined( 'ICL_SITEPRESS_VERSION' ) ) {
		add_action('ic_clone_page', 'i_clone_wpml_copy_translations', 10, 3);
		add_action('ic_i_clone', 'i_clone_wpml_copy_translations', 10, 3);
		add_action('shutdown', 'clone_wpml_string_packages', 11 );
	}
}
...
function i_clone_wpml_copy_translations($post_id, $post, $status = '') {
	global $sitepress;
	global $cloned_posts;
	
	remove_action('ic_clone_page', 'i_clone_wpml_copy_translations', 10);
	remove_action('ic_i_clone', 'i_clone_wpml_copy_translations', 10);
		...
		foreach ($translations as $code => $details) {
			if ($code != $current_language) {
				if ( $details->element_id ) {
					...
					$new_post_id = i_clone_create_clone( $translation, $status );
					$sitepress->set_element_language_details( $new_post_id, 'post_' . $translation->post_type, $new_trid, $code, $current_language );
				}
			}
		}
		$cloned_posts[ $post->ID ] = $post_id;
	}
}

function clone_wpml_string_packages() {
	global $cloned_posts;
	
	foreach ( $cloned_posts as $original_post_id => $i_clone_id ) {
		
		$original_string_packages = apply_filters( 'wpml_st_get_post_string_packages', false, $original_post_id );
		$new_string_packages      = apply_filters( 'wpml_st_get_post_string_packages', false, $i_clone_id );
